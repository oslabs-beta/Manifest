...
export default function UpdateMemLimitsForm(props: Props) {
  ...
  const handleSubmit = (event: { preventDefault: () => void }) => {
    ...
    const newHardLimitBytes = byteStringToBytes(
      formValues.hardLimit,
      formValues.hardLimitUnits
    );
    const newSoftLimitBytes = byteStringToBytes(
    ...
    if (newSoftLimitBytes > newHardLimitBytes)
      ddClient.desktopUI.toast.error(
        'The soft limit must be smaller than the hard limit. Please try again.'
      );
    //then check to make sure soft and hard limits are set  less than total memory avaliable to docker atm
    else if (newHardLimitBytes > totalDockerMem)
      ddClient.desktopUI.toast.error(
        'Limits must be less than the total memory allocated to Docker Desktop. Please try again.'
      );
    //then check to make sure soft/hard limits are inside of Dockers limits
    else if (newHardLimitBytes < 6000000 || newSoftLimitBytes < 6000000)
      ddClient.desktopUI.toast.error(
        'Limits must be greater than 6m. Please try again'
      );
